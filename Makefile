...
web: describo
	...
	tmux new-session -d -s standalone "cd client && while true; do yarn serve; done" \; split-window -h "cd server && while true; do USE_LOCAL_DICTS="True" pipenv run python starter.py; done" \; || true
...
ocis:
	...
	tmux new-session -d -s standalone "cd client && while true; do yarn serve; done" \; split-window -h "cd server && while true; do USE_LOCAL_DICTS="True" pipenv run python starter.py; done" \; || test
...
standalone: describo
	docker-compose -f client/dev/docker-compose.yml up -d
	tmux new-session -d -s standalone "cd client && while true; do yarn serve; done" \; split-window -h "cd server && while true; do USE_LOCAL_DICTS="True" pipenv run python starter.py; done" \;
	@echo Wait for 20 Seconds to boot everything up.
	@sleep 20
	@docker exec -it owncloud_server /bin/bash -c "occ app:enable oauth2 && occ app:enable rds"
	@echo Warning!!! You have to create a new oauth2 url and enter it in root .env file and configure RDS properly.

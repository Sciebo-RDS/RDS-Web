...
Aktuell verknüpft Sciebo RDS verschiedene Dienste, um Ihre Daten aus der Hochschulcloud heraus zu publizieren oder zu archivieren. Weitere Informationen dazu finden Sie auf der [offiziellen Webseite](https://www.research-data-services.org/de/)."""
        ...
        "Kontoverwaltung": {
            "Allgemein": "Sciebo RDS nutzt die Oauth2-Technologie. Hierbei werden im Namen des:der Nutzer:in Informationen mit Diensten ausgetauscht, ohne dabei Passwörter oder andere kritische Daten zu übermitteln. Ist die Dienstleistung durch Sciebo RDS nicht mehr erwünscht, können die Zugriffsrechte ohne Weiteres durch den:die Nutzer:in wieder entzogen werden. Für die Arbeit mit Sciebo RDS ist es nicht notwendig, alle angebotenen Dienste zu aktivieren. Sie können später jederzeit weitere Dienste hinzufügen oder nicht mehr genutzte Dienste entfernen.",
...
Currently, Sciebo RDS links various services to publish or archive your data from the university cloud. For more information, please visit the [official website](https://www.research-data-services.org/)."""
        ...
        "Account management": {
            "General": """Sciebo RDS uses the Oauth2 technology. Here, information is exchanged with services on behalf of the user without transmitting passwords or other critical data. If the service provided by Sciebo RDS is no longer desired, access rights can be easily revoked by the user.
...
To disable a service in Sciebo RDS, click "disconnect" on the corresponding service. Sciebo RDS will then disable access. All data transmitted to the service by the access right will be unaffected.""",
                "Manage / delete account": """The first time you use Sciebo RDS, a personal Sciebo RDS account will be created automatically. For ease of use, account access is controlled through the Sciebo university cloud. This means that as long as your Sciebo account is active, Sciebo RDS will also remain active, unless you manually deactivate Sciebo RDS.
